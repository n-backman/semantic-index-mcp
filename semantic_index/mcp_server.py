            ...
            try:
                ...
                if name == "graph_summary":
                    ...
                    if len(query) > _MAX_QUERY_LENGTH:
                        raise ValueError(f"query exceeds maximum length of {_MAX_QUERY_LENGTH}")
                    ...
                    result = engine.refactor_guardrail(symbol_id=str(arguments["symbol_id"]))
                elif name == "pre_edit_check":
                    query = str(arguments["query"])
                    if len(query) > _MAX_QUERY_LENGTH:
                        raise ValueError(f"query exceeds maximum length of {_MAX_QUERY_LENGTH}")
                    result = engine.pre_edit_check(
                        query=query,
                        kind=arguments.get("kind"),
                        depth=int(arguments.get("depth", 2)),
                    )
...
def serve(
    ...
    index_manager = AutoIndexManager(
        repo_root=repo_root,
        roots=roots,
        watch=watch,
        watch_interval=watch_interval,
    )
    index_manager.start(bootstrap=bootstrap_index or watch)
