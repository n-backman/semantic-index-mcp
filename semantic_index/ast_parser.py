...
from .schemas import SYMBOL_KIND_FUNCTION, SYMBOL_KIND_TYPE, SYMBOL_KIND_VARIABLE
...
def run_dump_parse(file_path: Path) -> str:
    try:
        proc = subprocess.run(
            ...
            capture_output=True,
            text=True,
            timeout=_SWIFTC_TIMEOUT_SECONDS,
        )
    except subprocess.TimeoutExpired as exc:
        raise ParseError(
            f"swiftc -typecheck -dump-parse timed out after {_SWIFTC_TIMEOUT_SECONDS}s "
            f"for {file_path}"
        ) from exc
...
def _extract_block(lines: list[str], start_index: int, marker: str) -> list[str]:
    ...
    idx = start_index + 1
    while balance > 0 and idx < len(lines) and idx - start_index < _MAX_BLOCK_LINES:
